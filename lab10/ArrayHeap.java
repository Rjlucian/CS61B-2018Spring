...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        for (int i = index; i > 1; i /= 2) {
            if (contents[i].priority() < contents[parentIndex(i)].priority()) {
    ...
    private void sink(int index) {
        validateSinkSwimArg(index);
        int child;
        for (int i = index; i * 2 <= size; i = child) {
            child = min(leftIndex(i), rightIndex(i));
            if (contents[i].priority() > contents[child].priority())  {
                swap(i, child);
            } else {
                return;
            }
